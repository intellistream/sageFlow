...
class JoinOperator : public Operator {
 public:
  JoinOperator(const std::string &name,
               std::function<bool(std::unique_ptr<VectorRecord> &,

                                  std::unique_ptr<VectorRecord> &)> &join_func,
               std::unique_ptr<LogicalPlan> &other_plan)
      : Operator(OperatorType::JOIN, name), join_func_(std::move(join_func)), other_plan_(std::move(other_plan)) {}

  auto process(std::unique_ptr<VectorRecord> &data) -> bool override { return join_func_(data, data); }
