...
auto candy::VectraFlow::query(std::unique_ptr<VectorRecord>& record, int k) -> std::vector<uint64_t> {
  ...
  for (unsigned long data : datas_) {
    auto rec = storage_manager_->getVectorByUid(data).get();
...
#pragma omp parallel for
  for (int i = 0; i < datas_.size(); ++i) {
    ...
    // VectraFlow 特有的计算方式
    auto dist =
        input_vector_square + selfquare[i] - 2 * storage_manager_->engine_->dotmultiply(rec->data_, local_rec->data_);
