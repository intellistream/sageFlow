...
std::vector<RecordView> VSJoinMethod::ExecuteEager(
    ...
    // 1. Query Local Index — own partition (lock-free, owned by this subtask)
    {
        ...
        if (subtask_index < local_ids.size()) {
            collectFromIndex(local_ids[subtask_index], query_record, seen, candidate_uids);
        }
        ...
        if (local_partitioner_ && num_partitions > 1 && local_num_probes_ > 1) {
            ...
            for (size_t pid : probes) {
                if (pid != subtask_index && pid < num_partitions) {
                    collectFromIndex(local_ids[pid], query_record, seen, candidate_uids);
                }
            }
        }
    }
    ...
    // 2. Query Global Index (shared, read-only, IVF)
    {
        ...
        collectFromIndex(global_target, query_record, seen, candidate_uids);
    }
    ...
    return resolveUidsToRecords(candidate_uids, target_state, subtask_index);
}
...
std::vector<RecordView> VSJoinMethod::resolveUidsToRecords(
    ...
    record_map.reserve(snapshot.size());
