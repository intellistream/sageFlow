...
sageFlow::VectorData::VectorData(const int32_t dim, const DataType type, char* data)
    : dim_(dim), type_(type) {
...
sageFlow::VectorData::VectorData(const int32_t dim, const DataType type)
    : dim_(dim), type_(type), data_(new char[dim * DATA_TYPE_SIZE[type]]) {}

sageFlow::VectorData::VectorData(const VectorData& other) : dim_(other.dim_), type_(other.type_) {
...
auto sageFlow::VectorData::operator==(const VectorData& other) const -> bool {
  if (dim_ != other.dim_) {
...
auto sageFlow::VectorData::operator!=(const VectorData& other) const -> bool { return !(*this == other); }

bool sageFlow::VectorData::Serialize(std::ostream &out) const {
...
bool sageFlow::VectorData::Deserialize(std::istream &in) {
...
void sageFlow::VectorData::printData(std::ostream &os) const {
  if (!data_) {
...
void sageFlow::VectorRecord::printRecord(std::ostream &os) const {
  os << "VectorRecord (uid: " << uid_ << ", timestamp: " << timestamp_ << ")" << std::endl;
  data_.printData(os);
}

sageFlow::VectorRecord::VectorRecord(const uint64_t& uid, const int64_t& timestamp, VectorData&& data)
  : uid_(uid), timestamp_(timestamp), data_(std::move(data)) {}

sageFlow::VectorRecord::VectorRecord(const uint64_t& uid, const int64_t& timestamp, const VectorData& data)
    : uid_(uid), timestamp_(timestamp), data_(data) {}

sageFlow::VectorRecord::VectorRecord(const uint64_t& uid, const int64_t& timestamp, int32_t dim, DataType type, char* data)
    : uid_(uid), timestamp_(timestamp), data_(dim, type, data) {}

auto sageFlow::VectorRecord::operator==(const VectorRecord& other) const -> bool {
  return uid_ == other.uid_ && timestamp_ == other.timestamp_ && data_ == other.data_;
}

bool sageFlow::VectorRecord::Serialize(std::ostream &out) const {
...
bool sageFlow::VectorRecord::Deserialize(std::istream &in) {
  uint64_t uid;
  int64_t ts;
