...
\begin{figure}[t]
	...
	\begin{tikzpicture}[font=\footnotesize]
		\node[draw, rounded corners, minimum width=3.35cm, minimum height=2.1cm, align=center] (leftbox) at (0,0)
		{\textbf{Static/Batch ANN}\\[0.12cm]
		Offline build + query phase\\
		Low state churn};

		\node[draw, rounded corners, minimum width=3.35cm, minimum height=2.1cm, align=center] (rightbox) at (4.35,0)
		{\textbf{Streaming Vector Join}\\[0.12cm]
		Insert + expire + probe\\
		Concurrent mutable state};

		\draw[->, thick] (1.75,0) -- (2.6,0);

		\node[draw, rounded corners, minimum width=7.85cm, minimum height=1.1cm, align=center] (tension) at (2.175,-1.85)
		{\textbf{Streaming multicore tension}\\
		Synchronization overhead \textbar\ boundary miss risk \textbar\ index staleness};
	\end{tikzpicture}
	\caption{Motivating contrast: streaming vector joins must preserve join quality while sustaining throughput and tail-latency under continuous insert/expire/probe updates.}
	\Description{A two-panel comparison between static or batch ANN and streaming vector joins. The static panel shows offline build with low churn; the streaming panel shows concurrent insert, expire, and probe. A bottom box summarizes the multicore tension: synchronization overhead, boundary misses, and index staleness.}
	\label{fig:intro-motivation}
\end{figure}

Our VSJoin design addresses this setting through three coupled mechanisms. First, \emph{Bounded-Staleness Read/Write Decoupling} separates foreground mutable updates from read-optimized global query structures, so insert/probe hot paths remain short while periodic rebuild controls staleness. Second, \emph{Budgeted Boundary Coverage Routing} uses LSH-based logical partitioning with bounded multicast fanout to reduce boundary misses without paying full broadcast cost. Third, \emph{Predictable Control Plane for Skew} applies lock-free reads over an RCU-style (read-copy-update) AssignmentTable and performs periodic, threshold-triggered bounded remapping based on sampled load signals. Together, these mechanisms target the three-way tension in Figure~\ref{fig:intro-motivation}: throughput, tail latency, and join quality.

\system in \sageflow serves as the implementation and evaluation substrate for VSJoin and for baseline execution paths (BruteForce, IVF, HNSW, HDR-Tree, LSH, ClusteredJoin, and S3J-style paths) under shared runtime assumptions. Consistent with the abstract, we scope all claims to the evaluated setting: verification remains the correctness layer, while routing and remapping are control heuristics designed for stable behavior rather than global optimality at every instant.

In summary, this paper makes the following \textbf{testable contributions}:
\begin{itemize}
	\item \textbf{Proposition P1 (Decoupled indexing under update churn).} We evaluate whether bounded-staleness read/write decoupling can preserve join quality while improving throughput--latency behavior under continuous insert/expire/probe pressure, relative to a single-path update/query coupling baseline in the same runtime.
	\item \textbf{Proposition P2 (Budgeted coverage versus routing extremes).} We evaluate whether bounded multicast routing provides a better quality--cost trade-off than unicast-only and full-broadcast extremes under evaluated fanout budgets and workload skew.
	\item \textbf{Proposition P3 (Predictable skew control with bounded overhead).} We evaluate whether RCU-published logical remapping with periodic heuristic triggering improves load stability while keeping control overhead bounded and the foreground join path lock-light.
	\item \textbf{Evidence scope and degradation boundaries.} We report implementation-aligned evidence with explicit boundaries, including degradation cases under high skew and tight fanout budgets, and limit conclusions to tested hardware, runtime settings, and workloads.
\end{itemize}

The rest of this paper is organized as follows. Section~\ref{sec:problem-statement} formalizes the problem and motivation. Section~\ref{sec:core-architecture} presents the core join architecture and method-specific execution paths. Section~\ref{sec:implementation-details} presents implementation details. Section~\ref{sec:results} starts with experimental methodology and then reports results. Section~\ref{sec:related-work} discusses related work, and Section~\ref{sec:conclusion} concludes.
% \section{Introduction}
% \label{sec:intro}
