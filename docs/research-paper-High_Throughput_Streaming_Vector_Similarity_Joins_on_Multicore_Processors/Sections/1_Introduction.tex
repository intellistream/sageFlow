...
Deep-learning applications increasingly depend on high-dimensional embedding vectors, making vector similarity joins a core primitive for real-time analytics. While prior work has focused on static or batched settings, streaming workloads require sliding-window semantics under continuous updates, frequent expirations, and multicore shared-memory concurrency.

In SageFlow, we approach this problem through a strategy-driven join architecture. A Join pipeline is configured by four coupled dimensions: algorithm, partition strategy, window-state type, and index strategy. This design allows a single runtime path to instantiate BruteForce, IVF, HNSW, HDRTree, LSH, ClusteredJoin, and S3J-style methods, while reusing common state and execution components.

The latest codebase reveals that not all combinations are equally mature. Two execution paths are currently the most stable in end-to-end tests and performance runs:
\begin{itemize}
    \item \textbf{Shared-index path:} RoundRobin + SharedWindowState, used by BruteForce/IVF/HNSW/HDRTree-style joins.
    \item \textbf{Centroid-partitioned path:} Centroid partitioning + PartitionedWindowState for ClusteredJoin, with cold-start and multicast controls.
\end{itemize}

This implementation experience highlights an important systems lesson: in streaming vector joins, correctness and recall depend on explicit compatibility constraints between partitioning and state models. SageFlow therefore enforces (or auto-corrects at runtime) key constraints such as partition/state compatibility and ClusteredJoin partition count alignment with runtime parallelism.

At the same time, several advanced features are still partial or under construction. For example, the VSJoin entry exists in configuration and factory wiring, but currently falls back to BruteForce behavior rather than a complete asynchronous VSJoin pipeline. Likewise, some state variants are present in interfaces but are not yet fully validated as production paths.

In summary, this paper makes the following contributions:
\begin{itemize}
    \item \textbf{Current-state architecture report:} We present the strategy-driven design used by SageFlow for streaming vector joins on multicore CPUs.
    \item \textbf{Validated execution scope:} We identify the currently stable paths and their operational constraints in real code and tests.
    \item \textbf{Gap analysis toward full VSJoin:} We enumerate missing components required for a complete partitioned asynchronous join pipeline.
    \item \textbf{Practical guidance:} We provide implementation-grounded recommendations for extending SageFlow without sacrificing recall or throughput.
\end{itemize}

\subsection{What remains to be completed}
To reach a full-featured VSJoin-style system, the following components are still important:
\begin{itemize}
    \item \textbf{VSJoin core execution path:} replace the current factory fallback with a real VSJoin method implementation (partition-local candidate generation, boundary handling, and verifier path).
    \item \textbf{Asynchronous coordination layer:} complete coordinator/async candidate generation/verifier modules and integrate them with RuntimeContext and backpressure behavior.
    \item \textbf{State model hardening:} finish and validate TwoTier/PartitionedVector states under high-churn sliding windows, including correctness and recall stability tests.
    \item \textbf{Constraint enforcement unification:} move remaining runtime-only constraints into a consistent validator+runtime contract, avoiding silent misconfiguration.
    \item \textbf{Evaluation completeness:} add dedicated ablations for cold-start, multicast, and partition-count alignment, and report quality/throughput trade-offs under real datasets.
\end{itemize}
% \section{Introduction}
% \label{sec:intro}
