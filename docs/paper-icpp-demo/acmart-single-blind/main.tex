...
Stream processors provide scalable dataflow execution~\cite{akidau2015dataflow,carbone2015apache}, while vector indexes provide efficient approximate search~\cite{jegou2011product,johnson2019billion,malkov2020efficient}. Retrieval-augmented systems further show how language models benefit from external evidence~\cite{lewis2020rag,guu2020realm}, and recent work integrates retrieval into streaming settings~\cite{lu2025vectraflow,sankaradas2024streamingrag}. What is still awkward in many prototypes is the boundary between streaming state and AI orchestration. Semantic state is often rebuilt outside the component that observes the stream, so a demo can show an answer but not the live runtime evidence that made the answer possible.
