...
**SageFlow** is a vector-native stream processing engine designed for real-time, LLM-based generation tasks. It provides a declarative API to compose stateful vector operations within temporal windows, enabling fast and efficient updates to semantic context for dynamically changing datasets.

### Key Use Cases

- **Real-time LLM Generation**: Provide large language models with fresh, stateful context snapshots
- **Dynamic Context Maintenance**: Ideal for conversational AI with rapidly evolving context
- **Streaming Vector Analytics**: High-velocity semantic queries on vector data
- **Adaptive Recommendation Systems**: Real-time recommendations based on streaming events
...
- **Logging**: spdlog
- **Configuration**: tomlplusplus (TOML format)
...
│   ├── common/       # Common utilities and data types (VectorRecord, Response, etc.)
│   ├── compute_engine/  # Similarity/distance computation (cosine, L2)
│   ├── concurrency/  # Concurrency controller for thread-safe index access
│   ├── execution/    # Execution graph, vertices, runtime context, queues
│   ├── function/     # User-defined functions (Filter, Map, Join, etc.)
│   ├── index/        # Vector index implementations (HNSW, IVF, Bruteforce)
│   ├── operator/     # Stream operators (TopK, Filter, Join, Map, etc.)
│   ├── query/        # Query optimizer and planner
│   ├── state/        # Window state management (partitioned/shared)
│   ├── storage/      # Storage manager for vector records
│   ├── stream/       # Stream abstractions and data sources
