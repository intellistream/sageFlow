...
void SetupAndRunPipeline(const std::string &config_file_path) {
  ...
  const long num_stream_records = 100;  // 查询向量个数， 也就是流里的向量个数
  ...
  source_stream->topk(index_id, k)
      ->writeSink(
          std::make_unique<SinkFunction>("PerfSink", [&processed_count, &last_flag, &last_start_time, &latencies_us,
                                                      &records_ids, IsTestingCQR, &genanswer, &genanswer_flag](const std::unique_ptr<VectorRecord> &record) {
            if (!record) {
              //if (IsTestingCQR)
              //records_ids.emplace_back(-1);
              return;
            }  // Skip if  record is null
            if (IsTestingCQR) records_ids.emplace_back(record->uid_);
            if (genanswer_flag) {
  ...
  if (!latencies_us.empty()) {
    ...
    if (p95pos >= (int) latencies_us.size()) {
      p95pos = latencies_us.size() - 1;
    }
    if (p99pos >= (int) latencies_us.size()) {
