...
TEST(SimilarityTest, CalculateSimilarity) {
  auto vec1 = std::make_shared<candy::VectorRecord>("id1", candy::VectorData(std::vector<float>{1.0, 2.0, 3.0}), 1);
  auto vec2 = std::make_shared<candy::VectorRecord>("id2", candy::VectorData(std::vector<float>{4.0, 5.0, 6.0}), 2);
...
TEST(NormalizeTest, NormalizeVector) {
  auto vec = std::make_shared<candy::VectorRecord>("id1", candy::VectorData(std::vector<float>{3.0, 4.0}), 1);
...
TEST(TopKTest, FindTopK) {
  std::vector<std::shared_ptr<candy::VectorRecord>> records = {
      std::make_shared<candy::VectorRecord>("id1", candy::VectorData(std::vector<float>{1.0, 2.0}), 1),
      std::make_shared<candy::VectorRecord>("id2", candy::VectorData(std::vector<float>{3.0, 4.0}), 2),
      std::make_shared<candy::VectorRecord>("id3", candy::VectorData(std::vector<float>{5.0, 6.0}), 3),
  };

  auto top_k = candy::ComputeEngine::findTopK(
      records, 2, [](const std::shared_ptr<candy::VectorRecord>& record) { return record->timestamp_; });
