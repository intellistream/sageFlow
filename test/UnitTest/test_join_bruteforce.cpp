...
TEST_F(JoinBruteForceTest, BasicCorrectness) {
  ...
  SUCCEED() << "Bruteforce BasicCorrectness executed without timeout/crash."
...
TEST_F(JoinBruteForceTest, ThresholdBoundary) {
  ...
  // 仅验证pipeline能正常跑完（不超时不崩溃）
  SUCCEED() << "Bruteforce ThresholdBoundary executed without timeout/crash."
            << " TODO: add boundary correctness checks later.";
}
...
TEST_P(JoinMethodComparison, MethodConsistency) {
  ...
  // 仅验证两种方法都能正常跑完（不超时不崩溃）
  SUCCEED() << "JoinMethodComparison executed without timeout/crash."
            << " TODO: add method consistency checks later.";
}
