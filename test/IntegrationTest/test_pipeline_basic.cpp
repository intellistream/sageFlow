...
static void wait_until_processed_and_stable(size_t expected_left,
                                            size_t expected_right,
                                            std::chrono::seconds max_total_wait = std::chrono::seconds(60)) {
  using namespace std::chrono_literals;
  const auto deadline = std::chrono::steady_clock::now() + max_total_wait;
  for (;;) {
  ...
  // 输出稳定判定：连续一段时间 total_emits 不再增长
  const auto stable_window = 50ms;
  const auto max_wait_more = 5s;
  uint64_t last = JoinMetrics::instance().total_emits.load();
  auto stable_since = std::chrono::steady_clock::now();
  auto end_by = std::chrono::steady_clock::now() + max_wait_more;
...
// 自适应等待：仅判断输出稳定（用于无输入或不可预估输入计数的场景）
static void wait_until_stable_only(std::chrono::milliseconds stable_window = std::chrono::milliseconds(100),
                                   std::chrono::seconds max_total_wait = std::chrono::seconds(2)) {
  using namespace std::chrono_literals;
...
TEST_F(MultiThreadPipelineTest, ParallelJoinConsistency) {
  ...
  // 自适应等待处理完成并输出稳定
  wait_until_processed_and_stable(expected_left, expected_right, std::chrono::seconds(60));
    env_->stop();
    env_->awaitTermination();
  ...
  if (!results_by_parallelism.empty()) {
    const auto& ref = results_by_parallelism.front();
    for (size_t i = 1; i < results_by_parallelism.size(); ++i) {
      const auto& cur = results_by_parallelism[i];
      if (ref.empty()) {
        SUCCEED();
        continue;
      }
...
//              ->writeSink(std::move(sink_func), 1);
//
//   // 记录期望输入计数（在 move 之前，已于构建 Source 前获取）
//   const size_t expected_left = static_cast<size_t>(data_size);
//   const size_t expected_right = static_cast<size_t>(data_size);
//
