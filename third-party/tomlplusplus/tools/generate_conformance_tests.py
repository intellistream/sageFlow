            ...
            for key, val in self.values.items():
                ...
                if isinstance(val, (TomlPPTable, TomlPPArray)) and len(val) > 0:
                    s += (
                        "\n"
                        + indent
                        + "\t\t{},".format(python_value_to_tomlpp(str(key)))
                    )
                    s += " " + val.render(indent + "\t\t")
                    s += "\n" + indent + "\t"
                else:
                    s += "{}, {} ".format(
                        python_value_to_tomlpp(str(key)), python_value_to_tomlpp(val)
                    )
                s += "},"
            s += "\n" + indent + "}"
        return s
...
def load_tests(source_folder, is_valid_set, ignore_list=None):
    ...
    files = [
        (
            f,
            str(f)[strip_source_folder_len + 1 : -5]
            .replace("\\", "-")
            .replace("/", "-")
            .strip(),
        )
...
def load_burnsushi_tests(tests):
    ...
    tests["valid"]["burntsushi"] = load_tests(
        Path(root_dir, "valid"),
        True,
        (
...
def load_iarna_tests(tests):
    ...
    tests["valid"]["iarna"] = load_tests(
        Path(root_dir, "values"),
        True,
        (
...
def write_test_file(name, all_tests):
    ...
    test_file_path = Path(
        utils.entry_script_dir(),
        "..",
        "tests",
        rf"conformance_{sanitize(name.strip())}.cpp",
    ).resolve()
