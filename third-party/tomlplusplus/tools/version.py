...
if __name__ == "__main__":
    ...
    args = ArgumentParser(
        r"version.py",
        description=r"Sets the project version in all the necessary places.",
    )
    args.add_argument(r"version", type=str)
    args = args.parse_args()

    version = re.fullmatch(
        r"\s*[vV]?\s*([0-9]+)\s*[.,;]+\s*([0-9]+)\s*[.,;]+\s*([0-9]+)\s*", args.version
    )
    if not version:
    ...
    for path in (root / r"include/toml++/impl/version.hpp", root / r"toml.hpp"):
        text = read_text_file(path)
        text = re.sub(
            r"""(\s*#\s*define\s+TOML_LIB_MAJOR)\s+[0-9]+""", rf"\1 {version[0]}", text
        )
        text = re.sub(
            r"""(\s*#\s*define\s+TOML_LIB_MINOR)\s+[0-9]+""", rf"\1 {version[1]}", text
        )
        text = re.sub(
            r"""(\s*#\s*define\s+TOML_LIB_PATCH)\s+[0-9]+""", rf"\1 {version[2]}", text
        )
        write_text_file(path, text)
    ...
    for file in (r"README.md", r"docs/pages/main_page.md"):
        ...
        text = read_text_file(path)
        text = re.sub(
            r"""(toml(?:plusplus|\+\+|pp)\s*[/:^]\s*)[0-9](?:[.][0-9]){2}""",
            rf"\1{noop_sub}{version_str}",
            text,
            flags=re.I,
        )
        text = re.sub(
            r"""(GIT_TAG\s+)(?:v\s*)?[0-9](?:[.][0-9]){2}""",
            rf"\1v{version_str}",
            text,
            flags=re.I,
        )
