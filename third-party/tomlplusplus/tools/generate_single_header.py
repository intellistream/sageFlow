...
class Preprocessor:
    ...
    def __preprocess(self, incl):
        ...
        if len(self.__include_stack) > 1 and incl_normalized not in (
            r"impl/header_start.hpp",
            r"impl/header_end.hpp",
        ):
...
def main():
    ...
    if 1:
        for i in range(3):
            ...
            toml_h = re.sub(
                r"^\s*//\s*SPDX-License-Identifier:.+?$", "", toml_h, 0, re.I | re.M
            )
            # double blank lines
            toml_h = re.sub("\n(?:[ \t]*\n[ \t]*)+\n", "\n\n", toml_h)
            # magic comments
            ...
            toml_h = re.sub(
                rf"(?:{header_separator}{blank_line}*)+({header_separator})",
                r"\1",
                toml_h,
            )
    ...
    # read version number
    version_h = utils.read_all_text_from_file(
        Path(include_dir, "impl/version.hpp"), logger=True
    )
    match = re.search(
    ...
    if match is None:
        raise Exception(
            "could not find TOML_LIB_MAJOR, TOML_LIB_MINOR or TOML_LIB_PATCH impl/version.hpp"
        )
